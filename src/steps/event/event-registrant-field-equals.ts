...
export class EventRegistrantFieldEqualsStep extends BaseStep implements StepInterface {

  protected stepName: string = 'Check a field on a Goldcast event registrant';
  // tslint:disable-next-line:max-line-length
  protected stepExpression: string = 'the (?<field>[a-zA-Z0-9_-]+) field on goldcast registrant from event (?<eventId>[a-zA-Z0-9_-]+) should (?<operator>be set|not be set|be less than|be greater than|be one of|be|contain|not be one of|not be|not contain|match|not match) ?(?<expectation>.+)?';
  protected stepType: StepDefinition.Type = StepDefinition.Type.VALIDATION;
  protected actionList: string[] = ['check'];
  protected targetObject: string = 'Event Registrant';
  ...
  async executeStep(step: Step) {
    ...
    try {
      let data: any = await this.client.getEventRegistrant(eventId);
      data = data.data;
      ...
      if (data && data.hasOwnProperty(field)) {
        ...
        if (data && !data.hasOwnProperty(field)) {
          return this.fail(
            'Found the registrant with event id %s, but there was no %s field.',
            [eventId, field],
            records,
          );
        } else {
          return this.fail("Couldn't find a registrant associated with event id %s", [
            eventId,
          ]);
        }
      }
  ...
  createRecords(registrant: Record<string, any>, stepOrder = 1) {
    ...
    // Ordered Record
    records.push(this.keyValue(`registrant.${stepOrder}`, `Checked Registrant from Step ${stepOrder}`, registrant));
    return records;
  }
}
